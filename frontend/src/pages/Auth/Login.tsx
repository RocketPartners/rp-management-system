...
export default function Login() {
    ...
    const [useGoogleButton, setUseGoogleButton] = useState<boolean>(false);
    ...
    // Render Google's own button as fallback when FedCM is unavailable
    useEffect(() => {
        if (useGoogleButton && googleReady && googleButtonRef.current) {
    ...
    }, [useGoogleButton, googleReady]);

    const handleGoogleClick = () => {
        if (window.google) {
            window.google.accounts.id.prompt((notification) => {
                // If One Tap / FedCM is blocked, fall back to rendered button
                            ...
                            <CardContent>
                                ...
                                {GOOGLE_CLIENT_ID && (
                                    <div className="animate-fade-in animation-delay-750 mt-6">
                                        ...
                                        {useGoogleButton ? (
                                            /* Google's own rendered button (fallback when FedCM is blocked) */
                                            ...
                                            <Button
                                                ...
                                                disabled={!googleReady || googleProcessing || processing}
